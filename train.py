...
class CustomDataGenerator(Sequence):
    def __init__(self, datax, datay, n, batch_size):
        ...
        self.num_batches = min(len(self.burst_files), len(self.no_burst_files)) // batch_size

    def __len__(self):
        return self.num_batches

    def __getitem__(self, index):
        rand = np.random.randint(n, size=(batch_size))
    ...
    #         if data.shape != self.desired_shape:
    #             return None
    #         return data
    #     return None
...
if __name__ == '__main__':
    ...
    history = model.fit(train_generator, epochs=1000, steps_per_epoch=steps_per_epoch, callbacks=[cp_callback])
